...
def signal_to_returns(
    ...
    if plot_signals:
        ...
        fig, ax = plt.figure(figsize=(PLOT_WIDTH, PLOT_HEIGHT))
        ax.plot(prices.iloc[:, 0], label=f'{asset_name} Adj. Prices', alpha=0.5)

        # Plot buy signals
        buy_signals = trade_signals[trade_signals.iloc[:, 0] > 0].index
        ax.plot(buy_signals, prices.iloc[:, 0][buy_signals], '^', markersize=10, color='g', lw=0, label='Buy Signal')

        # Plot sell signals
        sell_signals = trade_signals[trade_signals.iloc[:, 0] < 0].index
        ax.plot(sell_signals, prices.iloc[:, 0][sell_signals], 'v', markersize=10, color='r', lw=0, label='Sell Signal')

        ax.set_title(f'{asset_name} Adj. Prices and Trading Signals ({strategy_name} Strategy)')
        ax.legend()
        ax.grid(True)
        ...
        if save_plot:
            plot_name_prefix = plot_name if plot_name else f"signals_{asset_name}_{strategy_name}"
    ...
    if plot_returns:
        ...
        fig, ax = plt.subplots(figsize=(12, 5))  # Define fig and ax
        ax.plot(strategy_cumulative.iloc[:, 0], label=f'{strategy_name} Strategy Cumulative Return')
        ax.plot(asset_returns_cumulative.iloc[:, 0], label=f'{asset_name} Buy and Hold Strategy Cumulative Return')

        ax.set_title(f'Cumulative Returns of the {strategy_name} vs. Buy and Hold Strategy')
        ax.legend()
        ax.grid(True)
        ...
        # Save the plot if requested
        if save_plot:
            plot_name_prefix = plot_name if plot_name else f"cumulative_returns_{strategy_name}_{asset_name}"
            save_figure(fig, plot_name_prefix)
...
def plot_roc_strategy(
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"roc_signals_{asset_name}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"roc_cumulative_returns_{asset_name}"
        save_figure(fig, plot_name_prefix)
...
def plot_sma_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"sma_crossover_{asset_name}_{short_window}_{long_window}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"sma_cumulative_returns_{asset_name}_{short_window}_{long_window}"
        save_figure(fig, plot_name_prefix)
...
def plot_boll_bands_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"bollinger_bands_{asset_name}_{ma_window}_{n_std_dev}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"bollinger_cumulative_returns_{asset_name}_{ma_window}"
        save_figure(fig, plot_name_prefix)
...
def plot_ema_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"ema_crossover_{asset_name}_{short_window}_{long_window}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"ema_cumulative_returns_{asset_name}_{short_window}_{long_window}"
        save_figure(fig, plot_name_prefix)
...
def plot_macd_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"macd_strategy_{asset_name}_{short_window}_{long_window}_{signal_window}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"macd_cumulative_returns_{asset_name}_{short_window}_{long_window}_{signal_window}"
        save_figure(fig, plot_name_prefix)
...
def plot_rsi_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"rsi_strategy_{asset_name}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"rsi_cumulative_returns_{asset_name}_{rsi_period}"
        save_figure(fig, plot_name_prefix)
...
def plot_mfi_strategy(prices: pd.DataFrame,
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"mfi_strategy_{asset_name}_{mfi_period}"
        save_figure(fig, plot_name_prefix)
    ...
    # Save the plot if requested
    if save_plot:
        plot_name_prefix = plot_name if plot_name else f"mfi_cumulative_returns_{asset_name}_{mfi_period}"
        save_figure(fig, plot_name_prefix)
